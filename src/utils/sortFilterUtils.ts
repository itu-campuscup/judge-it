 ...
 * Note: Returns a new array to avoid in-place mutation.
 * Performance Optimization: Uses Schwartzian Transform (map-sort-map) to reduce
 * timeToMilli calls from O(N log N) to O(N).
 * @param {Array} timeLogs - The list of time logs.
 ...
 * Note: Returns a new array to avoid in-place mutation.
 * Performance Optimization: Uses Schwartzian Transform (map-sort-map) to reduce
 * key transformation and timeToMilli calls from O(N log N) to O(N).
 * @param {Array} timeLogs - The list of time logs.
...
export const sortTimeLogsByHeatAndTime = (timeLogs: TimeLog[]): TimeLog[] => {
  return timeLogs
    .map((log) => ({
      ...
      time: timeToMilli(log.time || ""),
    }))
    .sort((a, b) => {
      // Use direct comparison for non-locale-sensitive IDs
