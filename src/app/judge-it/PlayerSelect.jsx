...
const PlayerSelect = ({ teams, selectedTeamId, selectedPlayer, setSelectedPlayer, players, teamPlayers = [], setTeamPlayers, selectPlayerString, setSelectPlayerString }) => {

  useEffect(() => {
    if (selectedTeamId) {
      console.log('selectedTeam', selectedTeamId);  
      const teamPlayerIds = getPlayerIdsGivenTeamId(selectedTeamId, teams);
      const teamPlayers = teamPlayerIds.map((id) => getPlayerGivenId(id, players));
      setTeamPlayers(teamPlayers);
    }
  }, [selectedTeamId, players, setTeamPlayers]);
  ...
  return (
    <FormControl fullWidth margin='normal' variant='filled' disabled={teamPlayers.length === 0}>
