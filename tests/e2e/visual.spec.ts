...
const removeDevToolsIndicator = async (page: Page) => {
  const devtoolsIndicator = page.locator("#devtools-indicator");
  if (await devtoolsIndicator.count())
    await devtoolsIndicator.evaluate((el: HTMLElement) => el.remove());
};
